...
class ConsumerToken
  ...
  def self.find_or_create_from_access_token(user, access_token)
    ...
    if user
      user.consumer_tokens.where(_type: to_s, token: access_token.token).first ||
        create!(_type: to_s, token: access_token.token, secret: secret, user: user)
    else
      user = User.where('consumer_tokens._type' => to_s, 'consumer_tokens.token' => access_token.token).first
      if user
