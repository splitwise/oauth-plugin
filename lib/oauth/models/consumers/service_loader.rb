...
if defined? ConsumerToken && defined? OAUTH_CREDENTIALS
  ...
  OAUTH_CREDENTIALS.each do |key, value|
    ...
    unless Object.const_defined?(class_name.to_sym)
      if File.exist?(File.join(File.dirname(__FILE__), 'services', "#{key}_token.rb"))
        Rails.logger.info File.join(File.dirname(__FILE__), 'services', "#{key}_token")
        require File.join(File.dirname(__FILE__), 'services', "#{key}_token")
      else
        begin
