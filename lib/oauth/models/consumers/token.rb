  ...
  module Models
    module Consumers
      module Token
        ...
        module ClassMethods
          def service_name
            @service_name ||= to_s.underscore.scan(/^(.*?)(_token)?$/)[0][0].to_sym
          end
          ...
          def find_or_create_from_access_token(user, access_token, new_token = nil)
            if access_token.class.ancestors.include?(Oauth2Token)
              ...
              token = if user
                        find_or_initialize_by_user_id_and_token(user.id, access_token.token)
                      else
                        find_or_initialize_by_token(access_token.token)
                      end
            end

            token = new_token ? set_details(new_token, access_token) : set_details(access_token, token)
          ...
          def set_details(access_token, token)
            ...
            if token.class.ancestors.include?(Oauth2Token)
              expires_in, expires_at = token.expiration_date(access_token)
            end
