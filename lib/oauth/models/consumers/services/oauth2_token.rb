...
class Oauth2Token < ConsumerToken
  ...
  def self.create_consumer(_options = {})
  ...
  def expired_and_existing?
    return true if !new_record? && (Time.now.to_i >= expires_at.to_i)
  ...
  def expiration_date(token)
    if !token.expires_in.nil? && !token.expires_at.nil?
      return token.expires_in, token.expires_at
    end
    if token.expires_at.nil? && !token.expires_in.nil?
      return token.expires_in, (Time.now.to_i + token.expires_in.to_i)
    end
    if token.expires_in.nil? && !token.expires_at.nil?
      return (token.expires_at.to_i - Time.now.to_i), token.expires_at
    end

    ['3600', (Time.now.to_i + 3600)]
  end
end
